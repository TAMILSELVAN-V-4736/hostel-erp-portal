...
export default function AdminMaintenanceApprovalsPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      await fetchWithAuth(`/maintenance/${id}/status`, {
        method: "PATCH",
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        ...
        >
          ...
          <p className="text-slate-400 text-sm">
            Review and manage student maintenance requests across hostels.
          </p>
        </motion.div>
      </div>
                  ...
                  requests.map((req, index) => (
                    ...
                    >
                      <td className="px-6 py-4 font-medium text-slate-200">
                        {req.student?.name || req.studentId || "Unknown Student"}
                      </td>
                      <td className="px-6 py-4 text-slate-400">
                        <span className="bg-slate-800/50 px-2 py-1 rounded-md text-xs border border-slate-700/50">
                          {req.category}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-slate-400 max-w-[250px] truncate">
                        <strong className="text-slate-300 font-medium mr-1">{req.title}:</strong>
                        {req.description}
                      </td>
